---
title: "4ndr0tools - Bunkr++ v4.3.0-Ψ"
date: 2026-04-24T15:58:32-05:00
draft: false
categories: ["PAYLOADS", "USERSCRIPTS"]
...
// @namespace    [https://github.com/4ndr0666/userscripts](https://github.com/4ndr0666/userscripts)
// @version      4.3.0-Ψ
// @author       4ndr0666
// @description  Part of 4ndr0tools: Canonical routing, auto-sort, hide visited, dl bypass, multi-tier m3u8 stream extract and native xor decryption
...
// @downloadURL  [https://github.com/4ndr0666/userscripts/raw/refs/heads/main/4ndr0tools%20-%20Bunkr%2B%2B.user.js](https://github.com/4ndr0666/userscripts/raw/refs/heads/main/4ndr0tools%20-%20Bunkr%2B%2B.user.js)
// @updateURL    [https://github.com/4ndr0666/userscripts/raw/refs/heads/main/4ndr0tools%20-%20Bunkr%2B%2B.user.js](https://github.com/4ndr0666/userscripts/raw/refs/heads/main/4ndr0tools%20-%20Bunkr%2B%2B.user.js)
// @grant        GM_download
// @grant        GM_addStyle
// @grant        GM_getResourceText
// @grant        GM_registerMenuCommand
// @connect      *
// @require      [https://code.jquery.com/jquery-3.6.0.min.js](https://code.jquery.com/jquery-3.6.0.min.js)
// @noframes
...
(function () {
    ...
    console.log('%c[4NDR0tools] Bunkr++ v4.2.1Ψ', 'color:#00E5FF; font-family:monospace; font-weight:bold;');
    ...
    let _sortExecuted  = false;  // closure-scoped, NOT on window
    let _debounceTimer = null;   // closure-scoped, NOT on window
    ...
    // Shared utility: ensure element is a positioning ancestor for absolute children
    ...
    // Explicit early-return guard prevents any self-redirect
    // on TARGET_DOMAIN. isAssetEndpoint check only runs on foreign hostnames.
    ...
    // Validate exactly two path segments /a/<id> before
    // enforcing sort params, preventing false redirects on future sub-routes.
    ...
    GM_addStyle(`
         ...
         * Shifts glyphs to the top-right to prevent interference with native player UI
         * that typically anchors to the bottom inside the video wrapper.
         */
        .psi-main-dl-glyph {
            top: 42px !important;
    ...
    // ==========================================
    const directMap = new Map(); // cache for intercepted URLs
    const origFetch = window.fetch;

    window.fetch = async function (...args) {
        const reqUrl = typeof args[0] === 'string' ? args[0] : (args[0]?.url || '');

        // Tier 2: Intercept the native player's API request and steal the decrypted payload
        ...
        // Tier 1 & Logging: Standard intercept
        const res = await origFetch.apply(this, args);
        try {
    ...
    function forceLargestFirst() {
        ...
        const sortInterval = setInterval(() => {
            ...
            const sizeBtn =
                document.querySelector('.btnSize, button[title*="size"], a[href*="sort=size"], .sort-size') ||
            ...
            if (sizeBtn) {
                ...
                setTimeout(() => {
                    ...
                    // Only positive ascending signals trigger a second click.
                    const isAscending =
                        txt.includes('↑') ||
                        cls.includes('-asc') ||
    ...
    // ==========================================
    // addVisited mutates only the in-memory Set. localStorage
    // is written exclusively in the beforeunload flush — not on every call.
    ...
    function addVisited(id) {
        ...
        if (cache.size > MAX_VISITED) {
            const [oldest] = cache; // first inserted value
            cache.delete(oldest);
        }
    }

    // Batch-flush dirty cache to localStorage on page exit
    ...
    function initVisitedTracker() {
        ...
        toggleBtn.title = 'Left-Click: Cycle Mode (DIM/HIDE/SHOW) | Right-Click: Purge Registry\nBuffer: 10,000 items max (FIFO — oldest entries evicted silently)';
    ...
    // RESTORED: Legacy copy fallback explicitly reinstated
    ...
    function robustCopy(text, overlay) {
        ...
        const onCopied = () => {
            if (isGlyph) {
                overlay.innerHTML = '<span style="font-size:14px;font-family:monospace;font-weight:bold;">✓</span>';
            } else {
        ...
        // RESTORED: Explicit visual feedback when fallback also fails
        const onFailed = () => {
            if (isGlyph) {
                overlay.innerHTML = '<span style="font-size:14px;font-family:monospace;font-weight:bold;">X</span>';
            } else {
                overlay.textContent = 'CLIPBOARD BLOCKED';
            }
    ...
    function createFileOverlay(url, element, idToMark) {
        ...
        // Auto-remove timer is paused on mouseenter so the
        // overlay persists while the user is actively interacting with it.
     ...
     * Checks directMap cache before any network request.
     * Uses DOMParser instead of raw regex on HTML text.
     *
    ...
    async function ghostFetch(href, fname) {
        // First-pass: check the live CDN URL map captured by the network proxy
        ...
        try {
            ...
            // DOMParser correctly handles SPA-rendered pages
            ...
            throw new Error('Download anchor not found in fetched DOM.');
        } finally {
            clearTimeout(timer);
        }
    }
    ...
    /**
     * getBunkrStreamUrl — Resolves the canonical streaming URL from /api/vs.
     * 4-Tier Waterfall Extraction Engine.
     * Replicates the XOR decryption cipher found in Bunkr's native player.enc.js.
     *
    ...
    async function getBunkrStreamUrl(slug) {
        ...
        // Tier 4: Autonomous Standalone XOR Decryption Engine
        ...
        try {
            ...
            if (data.encrypted) {
                // Native Decryption Engine based on player.enc.js
                const binaryString = atob(data.url);
                const keyBytes = new TextEncoder().encode(`SECRET_KEY_${Math.floor(data.timestamp / 3600)}`);
                const decryptedBytes = new Uint8Array(binaryString.length);
                for (let i = 0; i < binaryString.length; i++) {
    ...
    // =========================================================================
    // Shared helper: apply accessibility attributes and keyboard activation to a glyph
    ...
    function injectCaptureVector() {
        ...
        items.forEach(el => {
            ...
            if (!el.querySelector('.psi-stream-glyph')) {
                ...
                const activateStream = async (e) => {
                    // Right-click guard on stream glyph too
                    if (e.type === 'mousedown' && e.button !== 0) return;
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    // Validate that alphaId is an alphanumeric slug,
                    // not a raw numeric file ID, before calling the stream API.
                    if (/^\d+$/.test(alphaId)) {
                        console.warn(`[Ψ-4NDR0666] Stream glyph: alphaId "${alphaId}" appears to be a numeric file ID, not a slug. /api/vs requires an alphanumeric slug. Navigate to the asset view page to use the stream extractor.`);
                        streamGlyph.style.color       = 'var(--red)';
                        streamGlyph.style.borderColor = 'var(--red)';
                        setTimeout(() => {
                            streamGlyph.style.color       = '';
    ...
    function bootstrap() {
        ...
        // Also disconnect on SPA navigation events that do not
        // trigger beforeunload, then reconnect after the new content settles.
        const onSpaNav = () => {
            observer.disconnect();
            // Brief delay allows the new page's DOM to settle before reconnecting
            setTimeout(() => {
                if (document.body) {
                    observer.observe(document.body, { childList: true, subtree: true });
                    injectCaptureVector();
